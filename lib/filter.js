...
filter._stringOnlyOperator = (operator, attributeConfig) => {
  ...
  if (
    STRING_ONLY_OPERATORS.indexOf(operator) !== -1 &&
    attributeConfig._type !== 'string'
  ) {
...
filter._parseScalarFilterElement = (attributeConfig, scalarElement) => {
  ...
  const error = filter._stringOnlyOperator(
    splitElement.operator,
    attributeConfig
  );
  if (error) return { error };

  if (attributeConfig._settings) {
    // relationship attribute: no further validation
...
filter._parseFilterElement = (
  ...
  const helperResult = filter._parseFilterElementHelper(
    attributeConfig,
    filterElement
  );
...
filter.parseAndValidate = request => {
  ...
  for (const key in request.params.filter) {
    ...
    if (typeof filterElement === 'string')
      request.params.filter[key] = filterElement = filterElement.split(
        FILTER_SEPERATOR
      );

    if (!Array.isArray(filterElement) && filterElement instanceof Object)
    ...
    parsedFilterElement = filter._parseFilterElement(
      key,
      resourceConfig.attributes[key],
      filterElement
    );
