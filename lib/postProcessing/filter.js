    ...
    for (let j = 0; j < response.data.length; j++) {
      if (!filter._filterKeepObject(response.data[j], filters)) {
        debug.filter(
          ...
          JSON.stringify(response.data[j].attributes)
        );
...
filter._filterMatches = (filterElement, attributeValue) => {
  ...
  const filterFunction = {
    ...
    '≥': function filterGreaterThanEqual(attrValue, filterValue) {
      return attrValue >= filterValue;
    },
    '≤': function filterLessThanEqual(attrValue, filterValue) {
      return attrValue <= filterValue;
    },
...
filter._filterKeepObject = (someObject, filters) => {
  for (const filterName in filters) {
    ...
    if (
      someObject.attributes.hasOwnProperty(filterName) ||
      filterName === 'id'
    ) {
      let attributeValue = someObject.attributes[filterName];
      if (filterName === 'id') attributeValue = someObject.id;
      const attributeMatches = filter._attributesMatchesOR(
        attributeValue,
        whitelist
      );
      ...
      const relationshipMatches = filter._relationshipMatchesOR(
        relationships,
        whitelist
      );
